...
!  integer(i4b)             :: status(MPI_STATUS_SIZE)
  ...
  integer(i4b), allocatable :: requests(:)
  integer(i4b), allocatable :: stats(:,:)
  ...
  ! number of send/receive operations
  allocate(requests(6*nWorkers*2))
  allocate(stats(MPI_STATUS_SIZE,6*nWorkers*2))
  ...
  if (pid==MasterID) then
    ...
    do iw=1,nWorkers
      ...
      call mpi_isend(temp1(1:parms%K*N2),parms%K*N2,MPI_DOUBLE_PRECISION,iw,0,MPI_COMM_WORLD,requests(6*(iw-1)+1),ierr)
   
      ! send p 
      temp1 = reshape(HHData%p(:,iHH1:iHH2),(/parms%J*N2/))
      call mpi_isend(temp1,parms%J*N2,MPI_DOUBLE_PRECISION,iw,0,MPI_COMM_WORLD,requests(6*(iw-1)+2),ierr)
      deallocate(temp1)
 
      ! send market
      allocate(itemp1(parms%J*N2))
      itemp1(1:N2) = HHData%market(iHH1:iHH2)
      call mpi_isend(itemp1(1:N2),N2,MPI_INTEGER,iw,0,MPI_COMM_WORLD,requests(6*(iw-1)+3),ierr)

      ! send iNonZero
      itemp1(1:parms%K*N2) = reshape(HHData%iNonZero(:,iHH1:iHH2),(/parms%K*N2/))
      call mpi_isend(itemp1(1:N2*parms%K),N2*parms%K,MPI_INTEGER,iw,0,MPI_COMM_WORLD,requests(6*(iw-1)+4),ierr)

      ! end iZero
      itemp1               = reshape(HHData%iZero(:,iHH1:iHH2),(/parms%J*N2/))
      call mpi_isend(itemp1,N2*parms%J,MPI_INTEGER,iw,0,MPI_COMM_WORLD,requests(6*(iw-1)+5),ierr)

      ! end nNonZero
      itemp1(1:N2) = HHData%nNonZero(iHH1:iHH2)
      call mpi_isend(itemp1(1:N2),N2,MPI_INTEGER,iw,0,MPI_COMM_WORLD,requests(6*(iw-1)+6),ierr)
      deallocate(itemp1)
...
!      end do
    ...
    ! receive q
    call mpi_irecv(temp1(1:parms%K*N2),parms%K*N2,MPI_DOUBLE_PRECISION,MasterID,0,MPI_COMM_WORLD,requests(6*(nWorkers+pid-1)+1),ierr)
    HHData%q = reshape(temp1(1:parms%K*N2),(/parms%K,n2/))
   
    ! receive p 
    call mpi_irecv(temp1,parms%J*N2,MPI_DOUBLE_PRECISION,MasterID,0,MPI_COMM_WORLD,requests(6*(nWorkers+pid-1)+2),ierr)
    HHData%p = reshape(temp1,(/parms%J,N2/))
    deallocate(temp1)

    allocate(iTemp1(N2*parms%J))
    ! receive market
    call mpi_irecv(itemp1(1:N2),N2,MPI_INTEGER,MasterID,0,MPI_COMM_WORLD,requests(6*(nWorkers+pid-1)+3),ierr)
    HHData%market = itemp1(1:N2)

    ! receive iNonZero
    call mpi_irecv(itemp1(1:parms%K*N2),parms%K*N2,MPI_INTEGER,MasterID,0,MPI_COMM_WORLD,requests(6*(nWorkers+pid-1)+4),ierr)
    HHData%iNonZero = reshape(itemp1(1:parms%K*N2),(/parms%K,N2/))

    ! receive iZero
    call mpi_irecv(itemp1,parms%J*N2,MPI_INTEGER,MasterID,0,MPI_COMM_WORLD,requests(6*(nWorkers+pid-1)+5),ierr)
    HHData%iZero = reshape(itemp1,(/parms%J,N2/))

    ! receive nNonZero
    call mpi_irecv(itemp1(1:N2),N2,MPI_INTEGER,MasterID,0,MPI_COMM_WORLD,requests(6*(nWorkers+pid-1)+6),ierr)
    ...
    !end do
    print *,'Data received. Processor ',pid 
  end if
  call mpi_waitall(6*nWorkers*2,requests,stats,ierr)
  do i1=1,size(stats,2)
    print *,i1,stats(:,i1)
  end do
  print *,'Data transmission complete.'
  deallocate(stats,requests)
